...
- **排版固定双行**：无论有无翻译都按两行排版（两行各自占半栏高）；只有原文时原文占**上半行**、下半行留空。因此字号与位置不会随"有无翻译"跳变。
...
- **explorer 重启**：歌词窗口是独立顶层窗口（非任务栏子窗口），explorer 重启不会销毁它，
  ...
  托盘图标也会自动恢复：窗口收到系统广播 `TaskbarCreated` 时重新注册托盘图标并立即重测布局，
  因此重启 explorer 后托盘菜单（锁定/跟随主题/退出）照常可用。
  唯一未重建的是 UIA 结构变化事件处理器（注册在旧 explorer 元素上），由 2s 心跳兜底。
