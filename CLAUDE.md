...
- 颜色默认跟随系统主题(显式配置 > 主题默认 > 深色兜底;浅色 0xFF1A1A1A/0xB31A1A1A):`WM_SETTINGCHANGE(ImmersiveColorSet)` + 布局心跳 lightTheme 快照兜底,仅主线程应用;开关持久化 `HKCU\Software\Taskbar-Lyrics\ThemeFollow`;托盘操作后颜色接管锁生效(忽略管道重放,托盘“恢复管道颜色”或重启解除)。
